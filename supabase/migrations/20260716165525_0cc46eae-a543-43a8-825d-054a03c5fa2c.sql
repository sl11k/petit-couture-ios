...
-- 1. Marker column so reserve/release are one-shot per order.
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS stock_reserved_at timestamptz;

-- 2. reserve_order_inventory: also decrement products.stock (once).
...
BEGIN
  ...
  LOOP
    ...
    -- Per-warehouse reservation (only if not already reserved on this item).
    ...
    -- Always decrement products.stock (once per order) as the admin-visible source of truth.
...
-- 3. finalize_order_stock: on COD (no prior reserve) decrement products.stock; if already reserved, don't double-count.
  ...
  LOOP
    ...
    IF v_warehouse_id IS NOT NULL THEN
      IF v_was_reserved THEN
        -- convert reservation into an actual decrement
        UPDATE public.inventory
    ...
    -- products.stock: only decrement if this order didn't already reserve.
...
-- 4. release_order_inventory: restore products.stock (once), then clear reservation marker.
...
BEGIN
  ...
  -- Only restore if we actually decremented for this order.
  ...
  LOOP
    ...
    -- Release warehouse reservation if any.
