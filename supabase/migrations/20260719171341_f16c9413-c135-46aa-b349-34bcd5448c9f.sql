...
WITH pending_admin AS (
  SELECT id,
         row_number() OVER (ORDER BY created_at, recipient_phone) AS rn
  FROM public.notif_queue
  WHERE event_code = 'admin_new_order'
    AND audience = 'admin'
    AND status IN ('pending', 'retry')
    AND related_entity = 'order'
    AND created_at > now() - interval '24 hours'
)
UPDATE public.notif_queue q
SET scheduled_at = now() + make_interval(secs => 90 * pending_admin.rn),
    locked_at = NULL,
    locked_by = NULL,
    status = 'retry',
    updated_at = now()
FROM pending_admin
WHERE q.id = pending_admin.id;