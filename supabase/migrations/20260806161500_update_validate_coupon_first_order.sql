...
BEGIN
  ...
  IF coalesce(c.first_order_only, false) THEN
    IF _user_id IS NULL AND _customer_email IS NULL THEN
      RETURN QUERY SELECT false, 'first_order_requires_login', c.id, c.code, c.discount_type, c.discount_value, 0::numeric;
      RETURN;
    END IF;
    
    IF EXISTS (
      SELECT 1 FROM public.orders o
      WHERE ((_user_id IS NOT NULL AND o.user_id = _user_id) OR (_customer_email IS NOT NULL AND lower(o.customer_email) = lower(_customer_email)))
        AND o.status NOT IN ('cancelled', 'failed', 'refunded')
    ) THEN
      RETURN QUERY SELECT false, 'not_first_order', c.id, c.code, c.discount_type, c.discount_value, 0::numeric;
      RETURN;
    END IF;
  END IF;
