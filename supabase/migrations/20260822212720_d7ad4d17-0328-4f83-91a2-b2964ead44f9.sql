...
BEGIN
  ...
  ELSIF NEW.payment_status IN ('unpaid', 'pending', 'failed', 'expired') THEN
    ...
    SELECT
      ...
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'id', oi.id, 'product_id', oi.product_id, 'variantId', oi.variant_id,
          'name', oi.product_name, 'image', null, 'price', oi.unit_price,
          'qty', oi.qty, 'size', oi.size, 'color', oi.color, 'sku', oi.sku,
          'currency', NEW.currency
        ) ORDER BY oi.created_at)
        FROM public.order_items oi WHERE oi.order_id = NEW.id
      ), '[]'::jsonb),
      NEW.total, NEW.currency, true, false, 'payment',
      COALESCE(NEW.payment_method::text, 'checkout'), NEW.created_at,
    ...
    ON CONFLICT (session_id) DO UPDATE SET
      ...
      subtotal = EXCLUDED.subtotal, currency = EXCLUDED.currency,
      reached_checkout = true, converted = false, stage = 'payment',
