...
BEGIN
  ...
  -- split each session into visits on 30 minutes of inactivity
  gapped AS (
    SELECT *,
  ...
  -- fallback: derive product views from page_view on /product/<slug>
  t_products_paths AS (
  ...
  SELECT jsonb_build_object(
    ...
    'referral_clicks', (SELECT COALESCE(sum(count), 0) FROM t_refs),
    'direct_visits', (SELECT c FROM direct_visits),
    'devices', COALESCE((SELECT jsonb_agg(row_to_json(d)) FROM devices d), '[]'::jsonb),
    ...
    'top_products', COALESCE(
        NULLIF((SELECT jsonb_agg(row_to_json(pe)) FROM t_products_events pe), '[]'::jsonb),
        COALESCE((SELECT jsonb_agg(row_to_json(ps)) FROM t_products_slugs ps), '[]'::jsonb)
      ),
