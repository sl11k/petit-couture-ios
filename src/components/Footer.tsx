...
export function Footer() {
  ...
  return (
    <footer data-live-id="site-footer" className="bg-foreground text-background mt-12" dir={isRTL ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-6xl px-6 py-12">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-4">
          <div className="space-y-4">
            ...
            <p data-live-id="footer-about" className="text-[13px] leading-relaxed text-background/75">{about}</p>
          </div>

          <div>
            <h3 data-live-id="footer-shop-title" className="text-[11px] tracking-[0.2em] text-background/60 mb-4">
              {ar ? "تسوّق" : "Shop"}
            </h3>
            <ul className="space-y-2 text-[13px]">
              {dbCats.length > 0 ? (
                dbCats.slice(0, 8).map((c) => (
                  <li key={c.slug}>
                    <Link data-live-id={`footer-category-${c.slug}`} to="/category/$slug" params={{ slug: c.slug }} className="hover:text-gold transition">
          ...
          <div>
            <h3 data-live-id="footer-care-title" className="text-[11px] tracking-[0.2em] text-background/60 mb-4">
          ...
          <div>
            <h3 data-live-id="footer-contact-title" className="text-[11px] tracking-[0.2em] text-background/60 mb-4">
              {ar ? "تواصل" : "Get in touch"}
            </h3>
            <ul className="space-y-2.5 text-[13px]">
              <li className="flex items-start gap-2"><MapPin className="h-3.5 w-3.5 mt-1 shrink-0 text-gold" /><span data-live-id="footer-address">{address}</span></li>
              <li className="flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-gold" /><a data-live-id="footer-phone" href={`tel:${phone}`} dir="ltr" className="hover:text-gold">{phone}</a></li>
              <li className="flex items-center gap-2"><Mail className="h-3.5 w-3.5 text-gold" /><a data-live-id="footer-email" href={`mailto:${email}`} className="hover:text-gold">{email}</a></li>
            </ul>
            <div className="mt-4 flex items-center gap-3">
              <a data-live-id="footer-social-instagram" href={ig} target="_blank" rel="noopener" aria-label="Instagram" className="grid h-9 w-9 place-items-center rounded-xl border border-background/20 hover:bg-background/10 transition"><Instagram className="h-4 w-4" /></a>
              <a data-live-id="footer-social-whatsapp" href={wa} target="_blank" rel="noopener" aria-label="WhatsApp" className="grid h-9 w-9 place-items-center rounded-xl border border-background/20 hover:bg-background/10 transition"><MessageCircle className="h-4 w-4" /></a>
              <a data-live-id="footer-social-tiktok" href={tiktok} target="_blank" rel="noopener" aria-label="TikTok" className="grid h-9 w-9 place-items-center rounded-xl border border-background/20 hover:bg-background/10 transition">
        ...
        <div className="mt-10 pt-6 border-t border-background/15 flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] text-background/55 tracking-soft">
          <span data-live-id="footer-copyright">© {new Date().getFullYear()} {BRAND_NAME}. {ar ? "جميع الحقوق محفوظة." : "All rights reserved."}</span>
