...
const ADMIN_DYNAMIC_PATTERNS: Array<[(text: string) => boolean, (text: string) => string]> = [
  ...
  [(text) => /^المنتجات\s*\((\d+)\)$/.test(text), (text) => text.replace(/^المنتجات\s*\((\d+)\)$/, "Products ($1)")],
  [(text) => /^السلة\s*\((\d+)\)$/.test(text), (text) => text.replace(/^السلة\s*\((\d+)\)$/, "Cart ($1)")],
  [(text) => /^طلبات سابقة\s*\((\d+)\)$/.test(text), (text) => text.replace(/^طلبات سابقة\s*\((\d+)\)$/, "Previous orders ($1)")],
  [(text) => /^العناوين\s*\((\d+)\)$/.test(text), (text) => text.replace(/^العناوين\s*\((\d+)\)$/, "Addresses ($1)")],
  [(text) => /^المفضلة\s*\((\d+)\)$/.test(text), (text) => text.replace(/^المفضلة\s*\((\d+)\)$/, "Wishlist ($1)")],
  [(text) => /^سجل التواصل\s*\((\d+)\)$/.test(text), (text) => text.replace(/^سجل التواصل\s*\((\d+)\)$/, "Communication log ($1)")],
  [(text) => /^ملاحظات\s*\((\d+)\)$/.test(text), (text) => text.replace(/^ملاحظات\s*\((\d+)\)$/, "Notes ($1)")],
  [(text) => /^النوع:\s+(.+)$/.test(text), (text) => text.replace(/^النوع:\s+(.+)$/, "Type: $1")],
  [(text) => /^الموظف:\s+(.+)$/.test(text), (text) => text.replace(/^الموظف:\s+(.+)$/, "Staff: $1")],
];
  ...
  },
  {
    ...
    items: [
     ...
     { to: "/admin/shipping", labelAr: "الشحن", labelEn: "Shipping", icon: Truck },
      { to: "/admin/oto", labelAr: "تكامل OTO", labelEn: "OTO Integration", icon: Truck },
