...
function TextField({
  ...
  label: string;
  value?: string;
  onChange: (v: string, opts?: UpdateOpts) => void;
  multiline?: boolean;
  coalesceKey?: string;
}) {
    ...
    <div>
      ...
      {multiline ? (
        <Textarea
          rows={3}
          value={value ?? ""}
          onChange={(e) => onChange(e.target.value, { label: `تعديل ${label}`, key })}
        />
      ) : (
        <Input
          value={value ?? ""}
  ...
  return (
    <div className="space-y-2">
      ...
      {list.map((b, i) => (
        <div key={i} className="rounded-md border border-border p-2 space-y-2">
          ...
          <Input
            ...
            value={b.label_ar ?? ""}
            onChange={(e) =>
              onChange(list.map((x, j) => (j === i ? { ...x, label_ar: e.target.value } : x)))
            }
          />
          <Input
            ...
            value={b.label_en ?? ""}
            onChange={(e) =>
              onChange(list.map((x, j) => (j === i ? { ...x, label_en: e.target.value } : x)))
            }
          />
          <LinkPicker
            value={b.url}
            onChange={(url) => onChange(list.map((x, j) => (j === i ? { ...x, url } : x)))}
          ...
          <select
            ...
            onChange={(e) =>
              onChange(list.map((x, j) => (j === i ? { ...x, variant: e.target.value as any } : x)))
            }
          >
          ...
          <label className="flex items-center gap-2 text-xs">
            <Switch
              checked={b.newTab ?? false}
              onCheckedChange={(v) =>
                onChange(list.map((x, j) => (j === i ? { ...x, newTab: v } : x)))
              }
            />
            فتح في تبويب جديد
          </label>
      ...
      <Button
        ...
        className="w-full"
        onClick={() =>
          onChange([...list, { label_ar: "زر", label_en: "Button", url: "/", variant: "primary" }])
        }
      >
  ...
  return (
    <div className="space-y-1">
      ...
      <MediaUploader
        ...
        onChange={(url) => onChange({ ...(value ?? {}), url: url ?? undefined })}
      ...
      <Input
        ...
        value={value?.caption_ar ?? ""}
        onChange={(e) => onChange({ ...(value ?? {}), caption_ar: e.target.value })}
      />
      <Input
        placeholder="Caption (English)"
        value={value?.caption_en ?? ""}
        onChange={(e) => onChange({ ...(value ?? {}), caption_en: e.target.value })}
      />
      {allowLink && (
        <LinkPicker
          value={value?.link}
...
function TextStyleEditor({
  ...
  onChange: (v: TextStyleSettings | undefined) => void;
}) {
  const v = value ?? {};
  const set = (patch: Partial<TextStyleSettings>) => onChange({ ...v, ...patch });
  const color = /^#[0-9a-fA-F]{6}$/.test(v.color ?? "") ? v.color! : "#111111";
  return (
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Input
            ...
            max={100}
            placeholder="100"
            value={v.opacity != null ? Math.round(v.opacity * 100) : ""}
            onChange={(e) =>
              set({
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => set({ fontWeight: e.target.value as any })}
          >
...
function CategoryPicker({ value, onChange }: { value?: string; onChange: (slug: string) => void }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from("categories")
      .select("id,slug,name_ar,name_en,image_url")
      .eq("is_active", true)
      .order("display_order")
      .then(({ data }) => {
        if (active) {
          setItems((data ?? []) as CategoryChoice[]);
          setLoading(false);
        }
      });
    return () => {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {!loading && items.length === 0 && (
        <p className="text-[11px] text-destructive">لا توجد تصنيفات مفعلة.</p>
      )}
    </div>
  );
}
...
function ManualProductPicker({
    ...
    (async () => {
      ...
      for (let from = 0; active; from += pageSize) {
        const { data } = await supabase
          .from("products")
          .select("id,slug,name_ar,name_en,sku,image_url")
          .eq("status", "active")
          .eq("is_active", true)
  ...
  const filtered = useMemo(() => {
    ...
    const matches = needle
      ? items.filter((item) =>
          `${item.name_ar ?? ""} ${item.name_en ?? ""} ${item.sku ?? ""} ${item.slug}`
            .toLowerCase()
            .includes(needle),
        )
      : items;
    return [...matches].sort(
      (a, b) => Number(value.includes(b.slug)) - Number(value.includes(a.slug)),
    );
  }, [items, query, value]);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <Label className="text-xs">المنتجات المختارة</Label>
      ...
      <div className="max-h-72 overflow-y-auto rounded-md border border-border bg-background">
        {loading ? (
          <p className="p-5 text-center text-xs text-muted-foreground">جاري تحميل المنتجات…</p>
        ) : filtered.length === 0 ? (
          ...
          filtered.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-2 border-b border-border p-2 text-start last:border-0 ${selected ? "bg-primary/5" : "hover:bg-muted/50"}`}
              >
                {item.image_url ? (
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <small className="block truncate text-[10px] text-muted-foreground">
                    {item.name_en} · {item.sku || item.slug}
                  </small>
                </span>
                <span
                  className={`grid h-5 w-5 place-items-center rounded border ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}
                >
...
function SpacingFields({
  ...
  const setSpacing = (k: "paddingTop" | "paddingBottom", val: number | undefined) => {
    const label =
      k === "paddingTop" ? `Padding Top → ${val ?? 0}px` : `Padding Bottom → ${val ?? 0}px`;
    onChange(
      (cur) =>
        ({
          ...cur,
          settings: {
            ...(cur.settings ?? {}),
            spacing: { ...(cur.settings?.spacing ?? {}), [k]: val },
          },
        }) as Section,
  ...
  const setBg = (val: string | undefined) => {
    ...
    onChange(
      (cur) => ({ ...cur, settings: { ...(cur.settings ?? {}), backgroundColor: val } }) as Section,
      { label, key: `bg:${s.id}` },
    );
    notify?.(label);
  };
    ...
    onChange(
      (cur) =>
        ({
          ...
          settings: {
            ...(cur.settings ?? {}),
            visibility: { ...((cur.settings as any)?.visibility ?? {}), [k]: val },
          },
        }) as Section,
...
function TypographyFields({
  ...
  const setTypography = (
    patch: NonNullable<Section["settings"]>["typography"],
    label: string,
    key: string,
  ) => {
    onChange(
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="mt-1 flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setTypography({ color: e.target.value }, `Text color → ${e.target.value}`, "color")
            }
            className="h-9 w-12 rounded border border-border bg-transparent"
          />
          <Input
            ...
            onChange={(e) =>
              setTypography({ color: e.target.value || undefined }, "Text color", "color")
            }
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setTypography({ fontFamily: e.target.value as any }, "Font family", "fontFamily")
            }
          >
            <option value="inherit">Theme</option>
            <option value="serif">Luxury serif</option>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setTypography({ fontWeight: e.target.value as any }, "Font weight", "fontWeight")
            }
          >
            <option value="300">Light</option>
            <option value="400">Regular</option>
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setTypography({ textAlign: e.target.value as any }, "Text alignment", "textAlign")
          }
        >
    ...
    <div className="space-y-4 text-sm">
      ...
      {s.type === "hero" && (
        <>
          <TextField
            label="Eyebrow (ع)"
            value={s.content.eyebrow_ar}
            onChange={(v, opts) => updateContent({ eyebrow_ar: v }, opts)}
          />
          <TextField
            label="Eyebrow (EN)"
            value={s.content.eyebrow_en}
            onChange={(v, opts) => updateContent({ eyebrow_en: v }, opts)}
          />
          <TextField
            label="العنوان (ع)"
            value={s.content.title_ar}
            onChange={(v, opts) => updateContent({ title_ar: v }, opts)}
          />
          <TextField
            label="Title (EN)"
            value={s.content.title_en}
            onChange={(v, opts) => updateContent({ title_en: v }, opts)}
          />
          <TextField
            label="العنوان الفرعي (ع)"
            multiline
            value={s.content.subtitle_ar}
            onChange={(v, opts) => updateContent({ subtitle_ar: v }, opts)}
          />
          <TextField
            label="Subtitle (EN)"
            multiline
            value={s.content.subtitle_en}
            onChange={(v, opts) => updateContent({ subtitle_en: v }, opts)}
          />
          <div>
            <Label className="text-xs">المحاذاة</Label>
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.alignment ?? "center"}
          ...
          <div>
            ...
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.layout ?? "overlay"}
              onChange={(e) => updateContent({ layout: e.target.value })}
            >
              <option value="overlay">النص فوق الصورة</option>
              <option value="split">صورة ونص منقسمان</option>
            </select>
          </div>
          <ImageField
            label="صورة الخلفية"
            value={s.content.image}
            onChange={(v) => updateContent({ image: v })}
            allowLink={false}
          />
          <ButtonsEditor
            buttons={s.content.buttons}
            onChange={(b) => updateContent({ buttons: b })}
          />
        </>
      )}
      ...
      {s.type === "text_block" && (
        <>
          ...
          <TextField
            label="النص (ع)"
            multiline
            value={s.content.body_ar}
            onChange={(v, opts) => updateContent({ body_ar: v }, opts)}
          />
          <TextField
            label="Body (EN)"
            multiline
            value={s.content.body_en}
            onChange={(v, opts) => updateContent({ body_en: v }, opts)}
          />
          <div>
            <Label className="text-xs">المحاذاة</Label>
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.alignment ?? "left"}
              onChange={(e) => updateContent({ alignment: e.target.value })}
            >
              <option value="left">يسار</option>
              <option value="center">وسط</option>
          ...
          <ButtonsEditor
            buttons={s.content.buttons}
            onChange={(buttons) => updateContent({ buttons })}
          />
        </>
      )}
      ...
      {s.type === "image_text" && (
        <>
          ...
          <div>
            ...
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.imageSide ?? "left"}
      ...
      {s.type === "feature_grid" && (
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs">صور كل صف بالجوال</Label>
              <select
                className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
                value={s.content.mobileColumns ?? 1}
                onChange={(e) => updateContent({ mobileColumns: Number(e.target.value) })}
              >
            ...
            <div>
              <Label className="text-xs">نسبة الصورة</Label>
              <select
                className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
                value={s.content.imageAspectRatio ?? "4/3"}
          ...
          <TextStyleEditor
            label="النص تحت الصورة / عنوان البطاقة"
            value={s.content.titleStyle}
            onChange={(titleStyle) => updateContent({ titleStyle })}
          />
          <TextStyleEditor
            label="وصف البطاقة"
            value={s.content.descriptionStyle}
            onChange={(descriptionStyle) => updateContent({ descriptionStyle })}
          />
          <TextStyleEditor
            label="تعليق الصورة"
            value={s.content.captionStyle}
            onChange={(captionStyle) => updateContent({ captionStyle })}
          />
          <div className="space-y-2">
              ...
              <div key={c.id} className="rounded-md border border-border p-2 space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className="h-7 px-2 text-destructive"
                    onClick={() =>
                      updateContent((ct: any) => ({
                        cards: ct.cards.filter((x: any) => x.id !== c.id),
                      }))
                    }
                  >
                    <Trash2 className="h-3 w-3" />
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      cards: ct.cards.map((x: any) =>
                        x.id === c.id ? { ...x, title_ar: e.target.value } : x,
                      ),
                    }))
                  }
                />
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      cards: ct.cards.map((x: any) =>
                        x.id === c.id ? { ...x, description_ar: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Textarea
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      cards: ct.cards.map((x: any) =>
                        x.id === c.id ? { ...x, description_en: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Input
                  placeholder="أيقونة أو رمز مثل ✦"
                  value={c.icon ?? ""}
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      cards: ct.cards.map((x: any) =>
                ...
                <ImageField
                  ...
                  onChange={(image) =>
                    updateContent((ct: any) => ({
                      cards: ct.cards.map((x: any) => (x.id === c.id ? { ...x, image } : x)),
                    }))
                  }
                />
                <LinkPicker
                  ...
                  onChange={(url) =>
                    updateContent((ct: any) => ({
                      cards: ct.cards.map((x: any) => (x.id === c.id ? { ...x, link: url } : x)),
                    }))
                  }
              ...
              <div key={it.id} className="rounded-md border border-border p-2 space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      updateContent((ct: any) => ({
                        items: ct.items.filter((x: any) => x.id !== it.id),
                      }))
                    }
                  >
                    <Trash2 className="h-3 w-3" />
                ...
                <Input
                  placeholder="سؤال (ع)"
                  value={it.question_ar ?? ""}
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, question_ar: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, question_en: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Textarea
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, answer_ar: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Textarea
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, answer_en: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <LinkPicker
                  ...
                  onChange={(link) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) => (x.id === it.id ? { ...x, link } : x)),
                    }))
                  }
                  placeholder="رابط إضافي للإجابة"
                />
              </div>
            ))}
            <Button
              ...
              <div key={it.id} className="rounded-md border border-border p-2 space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, role_ar: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, role_en: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Textarea
                  rows={2}
                  placeholder="الاقتباس (ع)"
                  value={it.quote_ar ?? ""}
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, quote_ar: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <Textarea
                  ...
                  onChange={(e) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, quote_en: e.target.value } : x,
                      ),
                    }))
                  }
                />
                <MediaUploader
                  value={it.avatar ?? null}
                  onChange={(url) =>
                    updateContent((ct: any) => ({
                      items: ct.items.map((x: any) =>
                        x.id === it.id ? { ...x, avatar: url ?? undefined } : x,
                      ),
                    }))
                  }
                  bucket="content-media"
                ...
                <LinkPicker
                  ...
                  placeholder="رابط العميل أو القصة"
                />
              </div>
            ))}
            <Button
              ...
              onClick={() =>
                updateContent((ct: any) => ({
                  items: [
                    ...ct.items,
                    { id: nid("t"), name: "", quote_ar: "", quote_en: "" } as TestimonialItem,
                  ],
                }))
              }
            >
        ...
        <>
          ...
          <div>
            ...
            <Input
              ...
              min={1}
              max={24}
              value={s.content.limit ?? 6}
              onChange={(e) =>
                updateContent({ limit: Math.max(1, Math.min(24, Number(e.target.value) || 6)) })
              }
            />
          </div>
          <div>
          ...
          <div className="text-[11px] text-muted-foreground bg-muted/40 rounded p-2">
            يعرض تلقائياً التقييمات المعتمدة من <strong>إدارة التقييمات</strong>. أضف تقييمات بحالة
            "مقبول" لتظهر هنا.
          </div>
        </>
      )}
            ...
            <div key={it.id} className="rounded-md border border-border p-2 space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateContent((ct: any) => ({
                    items: ct.items.map((x: any) =>
                      x.id === it.id ? { ...x, value: e.target.value } : x,
                    ),
                  }))
                }
              />
              <Input
                ...
                onChange={(e) =>
                  updateContent((ct: any) => ({
                    items: ct.items.map((x: any) =>
                      x.id === it.id ? { ...x, label_ar: e.target.value } : x,
                    ),
                  }))
                }
              />
              <Input
                ...
                onChange={(e) =>
                  updateContent((ct: any) => ({
                    items: ct.items.map((x: any) =>
                      x.id === it.id ? { ...x, label_en: e.target.value } : x,
                    ),
                  }))
                }
              />
          ...
          <Button
            ...
            onClick={() =>
              updateContent((ct: any) => ({
                items: [
                  ...ct.items,
                  { id: nid("st"), value: "0", label_ar: "", label_en: "" } as StatItem,
                ],
              }))
            }
          >
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <select
                className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
                value={s.content.imageAspectRatio ?? "1/1"}
                onChange={(e) => updateContent({ imageAspectRatio: e.target.value })}
              >
                <option value="1/1">1:1</option>
                <option value="4/3">4:3</option>
          ...
          <div className="space-y-2">
              ...
              <div key={i} className="rounded-md border border-border p-2 space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      updateContent((c: any) => ({
                        images: c.images.filter((_: any, j: any) => j !== i),
                      }))
                    }
                  >
                    <Trash2 className="h-3 w-3" />
                ...
                <ImageField
                  ...
                  onChange={(image) =>
                    updateContent((c: any) => ({
                      images: c.images.map((x: any, j: any) => (j === i ? image : x)),
                    }))
                  }
                />
              </div>
            ...
            <Button
              ...
              variant="outline"
              className="w-full"
              onClick={() => updateContent((c: any) => ({ images: [...c.images, {}] }))}
            >
              <Plus className="h-3 w-3 me-1" /> إضافة صورة
            </Button>
          </div>
        </>
      )}
      ...
      {s.type === "before_after" && (
        <>
          ...
          <div className="rounded-lg border border-border p-3 space-y-3">
            ...
            <ImageField
              label="ارفع صورة قبل"
              value={s.content.beforeImage}
              onChange={(v) => updateContent({ beforeImage: v })}
              allowLink={false}
            />
            <TextField
              label="تسمية قبل (عربي)"
              value={s.content.beforeLabel_ar}
              onChange={(v, opts) => updateContent({ beforeLabel_ar: v }, opts)}
            />
            <TextField
              label="Before label (English)"
              value={s.content.beforeLabel_en}
              onChange={(v, opts) => updateContent({ beforeLabel_en: v }, opts)}
            />
          </div>
          <div className="rounded-lg border border-border p-3 space-y-3">
            ...
            <ImageField
              label="ارفع صورة بعد"
              value={s.content.afterImage}
              onChange={(v) => updateContent({ afterImage: v })}
              allowLink={false}
            />
            <TextField
              label="تسمية بعد (عربي)"
              value={s.content.afterLabel_ar}
              onChange={(v, opts) => updateContent({ afterLabel_ar: v }, opts)}
            />
            <TextField
              label="After label (English)"
              value={s.content.afterLabel_en}
              onChange={(v, opts) => updateContent({ afterLabel_en: v }, opts)}
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              max={800}
              step={20}
              value={s.content.imageHeight ?? 520}
      ...
      {s.type === "video" && (
        <>
          ...
          <div className="space-y-1">
            ...
            />
            <Input
              placeholder="أو رابط MP4 / WebM مباشر"
              value={s.content.videoUrl ?? ""}
              onChange={(e) => updateContent({ videoUrl: e.target.value })}
            />
          </div>
          <ImageField
            label="صورة الغلاف"
            value={s.content.poster}
            onChange={(poster) => updateContent({ poster })}
            allowLink={false}
          />
          <TextField
            label="وصف الفيديو (عربي)"
            value={s.content.caption_ar}
            onChange={(v, opts) => updateContent({ caption_ar: v }, opts)}
          />
          <TextField
            label="Video caption (English)"
            value={s.content.caption_en}
            onChange={(v, opts) => updateContent({ caption_en: v }, opts)}
          />
          <LinkPicker
            value={s.content.link}
            onChange={(link) => updateContent({ link })}
            placeholder="رابط عند الضغط على الفيديو"
          />
          <div>
            <Label className="text-xs">نسبة العرض</Label>
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.aspectRatio ?? "16/9"}
          ...
          {(
            ...
            <label key={key} className="flex items-center justify-between text-xs">
              <span>{label}</span>
              <Switch
                checked={s.content[key] ?? (key === "controls" || key === "muted")}
                onCheckedChange={(value) => updateContent({ [key]: value })}
              />
            </label>
          ))}
        </>
      ...
      {s.type === "countdown" && (
        <>
          ...
          />
          <div>
            <Label className="text-xs">موعد انتهاء العد</Label>
            <Input
              type="datetime-local"
              value={s.content.targetDate?.slice(0, 16) ?? ""}
              onChange={(e) =>
                updateContent({
          ...
          <TextField
            label="النص بعد الانتهاء (عربي)"
            value={s.content.expiredText_ar}
            onChange={(v, opts) => updateContent({ expiredText_ar: v }, opts)}
          />
          <TextField
            label="Expired text (English)"
            value={s.content.expiredText_en}
            onChange={(v, opts) => updateContent({ expiredText_en: v }, opts)}
          />
          <ImageField
            label="صورة الخلفية"
            value={s.content.backgroundImage}
            onChange={(backgroundImage) => updateContent({ backgroundImage })}
            allowLink={false}
          />
          <ButtonsEditor
            buttons={s.content.button ? [s.content.button] : []}
      ...
      {s.type === "newsletter" && (
        <>
          ...
          />
          <TextField
            label="نص حقل البريد (عربي)"
            value={s.content.placeholder_ar}
            onChange={(v, opts) => updateContent({ placeholder_ar: v }, opts)}
          />
          <TextField
            label="Email placeholder (English)"
            value={s.content.placeholder_en}
            onChange={(v, opts) => updateContent({ placeholder_en: v }, opts)}
          />
          <TextField
            label="نص الزر (عربي)"
            value={s.content.buttonLabel_ar}
            onChange={(v, opts) => updateContent({ buttonLabel_ar: v }, opts)}
          />
          <TextField
            label="Button label (English)"
            value={s.content.buttonLabel_en}
            onChange={(v, opts) => updateContent({ buttonLabel_en: v }, opts)}
          />
          <TextField
            label="رسالة النجاح (عربي)"
            value={s.content.successText_ar}
            onChange={(v, opts) => updateContent({ successText_ar: v }, opts)}
          />
          <TextField
            label="Success message (English)"
            value={s.content.successText_en}
            onChange={(v, opts) => updateContent({ successText_en: v }, opts)}
          />
          <LinkPicker
            value={s.content.privacyUrl}
            onChange={(privacyUrl) => updateContent({ privacyUrl })}
            placeholder="رابط سياسة الخصوصية"
          />
          <Input
            value={s.content.actionUrl ?? ""}
            onChange={(e) => updateContent({ actionUrl: e.target.value })}
            placeholder="Webhook اختياري لاستقبال البريد"
          />
          <ImageField
            label="صورة الخلفية"
            value={s.content.backgroundImage}
      ...
      {s.type === "button" && (
        <>
          ...
          <div>
            ...
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.button?.variant ?? "primary"}
              onChange={(e) =>
                updateContent((c: any) => ({
                  button: { ...(c.button ?? {}), variant: e.target.value },
                }))
              }
            >
              <option value="primary">أساسي</option>
          ...
          <div>
            ...
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.shape ?? "rounded"}
              onChange={(e) => updateContent({ shape: e.target.value })}
            >
              <option value="square">مربع</option>
              <option value="rounded">دائري الزوايا</option>
          ...
          <label className="flex items-center gap-2 text-xs">
            <Switch
              checked={s.content.fullWidth ?? false}
              onCheckedChange={(v) => updateContent({ fullWidth: v })}
            />
            عرض كامل
          </label>
        </>
      ...
      {s.type === "banner" && (
        <>
          ...
          <div>
            ...
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.height ?? "md"}
              onChange={(e) => updateContent({ height: e.target.value })}
            >
              <option value="sm">قصير</option>
              <option value="md">متوسط</option>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => updateContent({ shape: e.target.value })}
            >
              <option value="square">حواف حادة</option>
              <option value="rounded">حواف ناعمة</option>
              <option value="pill">بيضاوي</option>
            </select>
          </div>
          <div>
            <div className="flex items-center justify-between">
              <Label className="text-xs">شفافية التغطية</Label>
              <span className="text-xs text-muted-foreground">
                {Math.round((s.content.overlay ?? 0.35) * 100)}%
              </span>
            </div>
            <input
              type="range"
              ...
              max={1}
              step={0.05}
              value={s.content.overlay ?? 0.35}
              onChange={(e) => updateContent({ overlay: Number(e.target.value) })}
              className="w-full accent-primary"
            />
          </div>
          <div>
            ...
            <input
              type="color"
              value={
                /^#[0-9a-fA-F]{6}$/.test(s.content.textColor ?? "")
                  ? s.content.textColor
                  : "#ffffff"
              }
      ...
      {s.type === "product_grid" && (
        <>
          ...
          {s.content.source === "category" && (
            <CategoryPicker
              value={s.content.categorySlug}
              onChange={(categorySlug) => updateContent({ categorySlug })}
            />
          )}
          {s.content.source === "manual" && (
            <ManualProductPicker
              value={s.content.productSlugs ?? []}
              onChange={(productSlugs) => updateContent({ productSlugs })}
            />
          )}
          <div>
            ...
            <Input
              ...
              min={1}
              max={48}
              value={s.content.limit ?? 8}
              onChange={(e) =>
                updateContent({ limit: Math.max(1, Math.min(48, Number(e.target.value) || 8)) })
              }
            />
          </div>
          <div>
          ...
          {(s.content.layout ?? "grid") === "grid" && (
            <div>
              <Label className="text-xs">عدد الأعمدة</Label>
              <select
                className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
                value={s.content.columns ?? 4}
                onChange={(e) => updateContent({ columns: Number(e.target.value) })}
              >
                <option value={2}>2</option>
                <option value={3}>3</option>
          ...
          {(s.content.layout ?? "grid") === "grid" && (
            <div>
              <Label className="text-xs">منتجات كل صف بالجوال</Label>
              <select
                className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
                value={s.content.mobileColumns ?? 2}
                onChange={(e) => updateContent({ mobileColumns: Number(e.target.value) })}
              >
                <option value={1}>1</option>
                <option value={2}>2</option>
          ...
          {s.content.layout === "carousel" && (
            <>
              <div>
                ...
                <select
                  className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
                  value={s.content.carouselCardSize ?? "medium"}
                  onChange={(e) => updateContent({ carouselCardSize: e.target.value })}
                >
                  <option value="compact">صغير — منتجات أكثر ظاهرة</option>
              ...
              <label className="flex items-center gap-2 text-xs">
                <Switch
                  checked={s.content.showNavigation !== false}
                  onCheckedChange={(showNavigation) => updateContent({ showNavigation })}
                />
                إظهار أسهم التنقل
              </label>
            </>
          ...
          <div>
            ...
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.cardShape ?? "rounded"}
          ...
          <div>
            <Label className="text-xs">نسبة صورة المنتج</Label>
            <select
              className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm"
              value={s.content.imageAspectRatio ?? "1/1"}
              onChange={(e) => updateContent({ imageAspectRatio: e.target.value })}
          ...
          <TextStyleEditor
            label="اسم المنتج"
            value={s.content.productNameStyle}
            onChange={(productNameStyle) => updateContent({ productNameStyle })}
          />
          <TextStyleEditor
            label="سعر المنتج"
            value={s.content.productPriceStyle}
            onChange={(productPriceStyle) => updateContent({ productPriceStyle })}
          />
          <label className="flex items-center gap-2 text-xs">
            <Switch
              checked={s.content.showPrice !== false}
      ...
      {s.type === "divider" && (
        <>
          ...
          <div>
            ...
            <input
              type="color"
              value={/^#[0-9a-fA-F]{6}$/.test(s.content.color ?? "") ? s.content.color : "#e5e7eb"}
      ...
      {s.type === "spacer" && (
        <div>
          <div className="flex items-center justify-between">
            <Label className="text-xs">الارتفاع</Label>
            <span className="text-xs text-muted-foreground">{s.content.height ?? 40}px</span>
          </div>
          <input
            type="range"
            min={4}
      ...
      {s.type === "html" && (
        <>
          ...
          <p className="text-[11px] text-muted-foreground">
            ⚠️ HTML يُحقن مباشرة في الصفحة — استخدم بحذر.
          </p>
        </>
      )}
        ...
        <div className="space-y-3">
          <div className="rounded-md border border-amber-300/40 bg-amber-50/50 dark:bg-amber-950/20 p-3 text-xs space-y-2">
            ...
            <p className="text-muted-foreground">
              لتعديل كل عنصر بشكل منفصل (عنوان، صور، أزرار، بطاقات...)، حوّله إلى أقسام قابلة
              للتعديل.
            </p>
          </div>
          {onConvertLegacy && (
