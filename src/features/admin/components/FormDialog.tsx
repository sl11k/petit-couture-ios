    ...
    switch (f.type) {
      case "number": {
        let numSchema: z.ZodTypeAny = z.number({
          invalid_type_error: ar ? "رقم غير صحيح" : "Invalid number",
        });
        if (f.min !== undefined)
          numSchema = (numSchema as z.ZodNumber).min(
            f.min,
            ar ? `الحد الأدنى ${f.min}` : `Min ${f.min}`,
          );
        if (f.max !== undefined)
          numSchema = (numSchema as z.ZodNumber).max(
            f.max,
            ar ? `الحد الأقصى ${f.max}` : `Max ${f.max}`,
          );
        s = z.preprocess(
...
async function syncJunction(
  cfg: NonNullable<NonNullable<FormFieldDef["lookup"]>["junction"]>,
  ownerId: string,
...
async function syncProductSizes(productId: string, entries: SizeEntry[]) {
  const clean = entries
    .map((e) => {
      const sizeAr = (e.size_ar || "").trim();
      const sizeEn = (e.size_en || "").trim();
      const canonical = (sizeEn || e.size || sizeAr || "").trim();
      return { ...e, size: canonical, size_ar: sizeAr, size_en: sizeEn || canonical };
    })
    .filter((e) => (e.size || e.size_ar || e.size_en || "").trim() !== "");
  const { data: existing, error: exErr } = await (supabase as any)
    .from("product_variants")
    .select("id, attributes")
    .eq("product_id", productId);
  ...
  for (let i = 0; i < clean.length; i++) {
    ...
    const payload = {
      ...
      attributes: {
        kind: "size",
        size_ar: e.size_ar || null,
        size_en: e.size_en || e.size || null,
      },
    };
    if (e.id && existingIds.has(e.id)) {
      keptIds.add(e.id);
  ...
  if (clean.length > 0 || existingIds.size > 0) {
    ...
    const labels = activeRows
      .map((e) => (e.size_en || e.size || e.size_ar || "").trim())
      .filter(Boolean);
...
export function FormDialog({
  ...
  const [values, setValues] = useState<Record<string, any>>(() =>
    defaultsFrom(visibleFields, initialValues),
  );
  ...
  const variantsField = useMemo(
    () => visibleFields.find((f) => f.type === "productVariants"),
    [visibleFields],
  );
  const sizesField = useMemo(
    () => visibleFields.find((f) => f.type === "productSizes"),
    [visibleFields],
  );
  const attributesField = useMemo(
    () => visibleFields.find((f) => f.type === "productAttributes"),
    [visibleFields],
  );
  const junctionLookupFields = useMemo(
    () => visibleFields.filter((f) => f.type === "lookup" && f.lookup?.junction),
    [visibleFields],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await (supabase as any)
        .from("product_variants")
        .select(
          "id, color_name_ar, color_name_en, color_hex, color_image_url, size, sku, stock, is_active, sort_order, attributes",
        )
        .eq("product_id", rowId)
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await (supabase as any)
        .from("product_variants")
        .select("id, size, sku, price, compare_at_price, stock, is_active, sort_order, attributes")
        .eq("product_id", rowId)
        .order("sort_order", { ascending: true });
      const sizeRows = ((data ?? []) as any[])
        .filter((r) => r?.attributes?.kind === "size")
        .map((r) => ({
          ...r,
          size_ar: r?.attributes?.size_ar ?? "",
          size_en: r?.attributes?.size_en ?? r?.size ?? "",
        }));
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await (supabase as any)
        .from("product_attributes")
        .select(
          "id, attribute_key, value_ar, value_en, color_hex, image_url, stock, is_active, sort_order",
        )
        .eq("product_id", rowId)
        .order("sort_order", { ascending: true });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (warehouseStockField && productId) {
        try {
          await persistWarehouseStock(
            productId,
            (values[warehouseStockField.key] as WarehouseStockEntry[]) ?? [],
          );
        } catch (err: any) {
          setSaving(false);
          console.error("[FormDialog] warehouse stock failed", err);
          toast.error(
            err?.message || (ar ? "تعذر حفظ مخزون المستودعات" : "Failed to save warehouse stock"),
          );
          return;
        }
      }
      ...
      if (productId && junctionLookupFields.length) {
        for (const f of junctionLookupFields) {
          try {
            ...
            setSaving(false);
            console.error("[FormDialog] junction sync failed", { field: f.key, err });
            toast.error(
              err?.message ||
                `${ar ? f.label.ar : f.label.en}: ${ar ? "فشل الحفظ" : "save failed"}`,
            );
            return;
          }
        }
      ...
      if (variantsField && productId) {
        try {
          await syncProductVariants(productId, (values[variantsField.key] as VariantEntry[]) ?? []);
        } catch (err: any) {
          setSaving(false);
          console.error("[FormDialog] variants sync failed", err);
          toast.error(err?.message || (ar ? "تعذر حفظ الألوان" : "Failed to save colours"));
          return;
        }
      }
      ...
      if (sizesField && productId) {
        try {
          await syncProductSizes(productId, (values[sizesField.key] as SizeEntry[]) ?? []);
        } catch (err: any) {
          setSaving(false);
          console.error("[FormDialog] sizes sync failed", err);
          toast.error(err?.message || (ar ? "تعذر حفظ المقاسات" : "Failed to save sizes"));
          return;
        }
      }
      ...
      if (attributesField && productId) {
        try {
          await syncProductAttributes(
            productId,
            (values[attributesField.key] as AttributeEntry[]) ?? [],
          );
        } catch (err: any) {
          setSaving(false);
          console.error("[FormDialog] attributes sync failed", err);
          toast.error(
            err?.message || (ar ? "تعذر حفظ التصنيفات الفرعية" : "Failed to save attributes"),
          );
          return;
        }
      }
      ...
      toast.error(
        err?.message || (ar ? "حدث خطأ غير متوقع أثناء الحفظ" : "Unexpected error during save"),
      );
    } finally {
      setSaving(false);
    }
            ...
            {visibleFields.map((f) => {
              ...
              return (
                <div key={f.key} className={cn("space-y-1.5", isFull && "sm:col-span-2")}>
                  ...
                  {f.type === "productVariants" ? (
                    <VariantEditor
                      value={(Array.isArray(values[f.key]) ? values[f.key] : []) as VariantEntry[]}
                      onChange={(v) => setVal(f.key, v)}
                    />
                  ) : f.type === "productSizes" ? (
                    <SizeSkuEditor
                      value={(Array.isArray(values[f.key]) ? values[f.key] : []) as SizeEntry[]}
                      onChange={(v) => setVal(f.key, v)}
                      basePrice={Number(values["price"]) || 0}
                      currency={(values["currency"] as string) || "SAR"}
                    />
                  ) : f.type === "productAttributes" ? (
                    <AttributesEditor
                      value={
                        (Array.isArray(values[f.key]) ? values[f.key] : []) as AttributeEntry[]
                      }
                      onChange={(v) => setVal(f.key, v)}
                    />
